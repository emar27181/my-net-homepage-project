...
- **🎮 実装対象ページ限定ルール（最重要）**
  → **http://localhost:8765/ （トップページ = ゲームページ）のみ実装対象**
  → ユーザーが「何かを作って」「追加して」等の指示をした場合、`src/pages/index.astro` を編集する
  → index.astroはゲーム情報ページ（game.astro）として機能する
  → **他のページ（about, blog, dev, gallery, link等）は使用しない**
  → 使用しないページは `src/pages/unused/` ディレクトリに移動済み
...
- **プレイ中のゲーム情報表示ルール（最重要）**
  → Player Infoセクションの「基本情報」内に「プレイ中のゲーム」項目として表示
  → **FPS**: Valorant / Apex / OW2 / BF / COD / Fortnite(昔)
  → **ソシャゲ**: パズドラ / モンスト / ポケポケ / 雀魂
  → フォントサイズは必ず本文テキストと同じ `${FONT_SIZES.bodyText}` を使用
  → line-heightは1.8で統一、キーワードレベルのシンプルな表示
  → このルールは絶対に遵守すること
